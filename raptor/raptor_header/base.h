...
void raptor_send_object(raptor* obj,sockinfo* sock){
    ...
    uint32_t network_order = htonl(obj->file_size_bytes); 
...
void raptor_send_block(raptor* obj,sockinfo* sock,uint16_t block_no){
    byte** data = (byte**) calloc(sizeof(byte*),obj->source_symbols_per_block );
    byte** int_symb;
    int32_t n;
    uint16_t symb_id=0;
    ...
    byte* buffer = (byte*) calloc(sizeof(byte),obj->symbol_size_bytes +header_size);

    for(uint32_t i=0;i<obj->source_symbols_per_block ;i++){
        data[i] = (byte*)calloc(sizeof(byte),obj->symbol_size_bytes +header_size+1);
        fread(data[i]+header_size,1,obj->symbol_size_bytes ,sock->fp);
        raptor_print(data[i],obj->symbol_size_bytes +header_size);
    }
    for(uint32_t i=obj->source_symbols_per_block ;i<obj->total_precoding_symbols ;i++)data[i] = (byte*) calloc(sizeof(byte),obj->symbol_size_bytes +header_size+1);

    // byte** enc_data;// = raptor_encode();
    for(i=0;i<obj->source_symbols_per_block ;i++){
        data[i][0] = block_no >> 8;
        data[i][1] = block_no & 255;
        data[i][2] = symb_id >> 8;
        data[i][3] = symb_id++;
        if((n=sendto(sock->send_fd,data[i],obj->symbol_size_bytes +header_size,0,(SA*)sock->send_addr,sock->send_addr_len))==-1)perror("sendto src");
    }
    ...
    allocate_gf2matrix(G_LT,obj->total_precoding_symbols ,obj->total_precoding_symbols );
    uint32_t* esi = (uint32_t*) malloc(sizeof(uint32_t)*obj->total_precoding_symbols );
    for(uint32_t i=0;i<obj->total_precoding_symbols ;i++)esi[i] = i+obj->source_symbols_per_block ;
    raptor_build_LT_mat(obj->total_precoding_symbols ,obj,G_LT,esi);
    ...
    for(uint32_t i=0;i<obj->total_precoding_symbols ;i++){
        ...
        for(uint32_t j=0;j<obj->total_precoding_symbols ;j++){
            if(get_entry(G_LT,i,j))xor(buffer+header_size,buffer+header_size,int_symb[j],obj->symbol_size_bytes );
        }
        buffer[0] = (block_no >> 8) | 128;
        buffer[1] = block_no & 255;
        buffer[2] = symb_id >> 8;
        buffer[3] = symb_id++;
        if((n=sendto(sock->send_fd,buffer,obj->symbol_size_bytes +header_size,0,(SA*)sock->send_addr,sock->send_addr_len))==-1)err_quit("sendto enc");
        }
}
...
void raptor_recieve_block(raptor* obj,sockinfo* sock,uint16_t block_no,uint32_t symbols_count){
    ...
    for(uint32_t i=0;i<symbols_count;i++)recieved_data[i] = (byte*) calloc(sizeof(byte),obj->symbol_size_bytes );
    while(received_symbols < symbols_count){
        ...
        memcpy(recieved_data[received_symbols],sock->buffer+header_size,obj->symbol_size_bytes );
        received_symbols++;
    }
    ...
    allocate_gf2matrix(gauss_mat,symbols_count,obj->source_symbols_per_block );
    gf2matrix* dummy_row = (gf2matrix*) malloc(sizeof(gf2matrix));
    allocate_gf2matrix(dummy_row,1,obj->total_precoding_symbols );
    uint32_t L_ = obj->total_precoding_symbols ;
    while(!is_prime(L_))L_++;
    for(uint32_t i=0;i<symbols_count;i++){
        if(ESI[i]<obj->source_symbols_per_block ){
            D[i] = 1;
    ...
    for(uint32_t i=0;i<obj->source_symbols_per_block ;i++)printf("%hhn ",recieved_data[i]);
}
